...
# form表单传过来的数据为 name:sku_id:sku.id 获取方式为：request.POST.get('sku_id')
...
# 订单事物
# /order/commit 需要参数 订单日期2016-8-21 17:36:24 订单号56872934 sku_id count price 小计
class OrderCommitView(View):
    ...
    def post(self,request):
        ...
        print(sku_ids,addr_id,pay_method)
        ...
        # 所有关于数据库的操作都要放到try中  一单有错误  就回滚失误
        try:
            ...
            for sku_id in sku_ids:
                ...
                for i in range(3):
                    ...
                    # update方法返回数字，代表更新的行数 如果是0表示没有更新
        ...
        return JsonResponse({'res':5,'errmsg':'建立订单成功'})
...
class OrderCheckView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # 参数校验
        if not all([order_id]):
            return HttpResponse({'res': 1, 'errmsg': '缺少参数'})
        print(order_id)
        # 校验订单id
        try:
            order = OrderInfo.objects.get(order_id=order_id,
...
class CommentView(LoginRequiredMixin,View):
    ...
    def get(self,request,order_id):
        ...
        # 获取订单商品信息
        order.skus = OrderGoods.objects.filter(order_id=order_id)
    ...
    def post(self,request,order_id):
        """提交评论"""
        # 提交评论改回status 为已评论
        # 将品论的内容存入good表 OrderGoods 中 comment 添加评论
        # 返回页面为'user:order',kwargs={'page':1}

        # 参数 order sku  conmment内容 评论人user_name 评论时间
        user = request.user
