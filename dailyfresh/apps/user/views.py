...
class UserInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        obj = Address.objects.get(user=user,is_default=True) # filter 拿到的是一个返回结果集合
        ...
        # 拿出键为history_userid 的商品编号sku_id
        ...
        return render(request, 'user_center_info.html', temp)
...
class UserOrderView(LoginRequiredMixin,View):
    ...
    def get(self,request,page):
        ...
        for order in orders:
            # 获取订单商品的信息
            # 给order对象增加属性order_skus，包
            order.order_skus = OrderGoods.objects.filter(order=order)
            # 便利商品信息
        ...
        if num_pages < 5:
            ...
            # 组织上下文
        context = {
            'order_page': order_page, # 当前页面的所有内容
            'pages': pages,
            'page': 'order' # 用来区分页面是个人信息 全部订单 收货地址
        }
